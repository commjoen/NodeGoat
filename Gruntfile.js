...
var JS_FILES = ["Gruntfile.js", "app/assets/js/**", "config/config.js",
  "app/data/**/*.js",
...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    nodemon: {
      dev: {
        options: {
          ...
          watchedFolders: ["app/data", "app/routes", "app/assets",
            "app/views", "app/views/tutorial"
          ],
          debug: true,
          delayTime: 1,
          env: {
    ...
    if: {
      testSecurityDependenciesInstalled: {
        options: {
          test: function() {
            ...
            try {
              ...
              console.log(
                "If this fails, please try installing manually,");
              console.log("there may be some help here:");
              console.log(
                "https://github.com/vuejs/vue-router/issues/261#issuecomment-218618180"
              );
              throw e;
            }
          }
        ...
        ifFalse: ["npm-install:chromedriver@^2.21.2",
          "mochaTest:security"
        ]
      }
    },
  ...
  grunt.registerTask("db-reset", "(Re)init the database.", function(arg) {
    ...
    var cmd = process.platform === "win32" ? "NODE_ENV=" + finalEnv +
      " & " : "NODE_ENV=" + finalEnv + " ";
  ...
  // Security test task.
  grunt.registerTask("testsecurity", ["env:test",
    "if:testSecurityDependenciesInstalled"
  ]);
