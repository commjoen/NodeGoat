...
test.describe(zapTargetAppRoute + " regression test suite", function() {
  ...
  test.it(
    "Should not exceed the decided threshold of vulnerabilities known to Zap",
    function(done) {
      var contextId = 1;
      var userId;
      var maxChildren = 1;
      ...
      var onCompletion = function(error,
        resultsFromAllAsyncSeriesFunctions) {
        if (!error)
          console.log(
            resultsFromAllAsyncSeriesFunctions[
              resultsFromAllAsyncSeriesFunctions.length - 1].description
          );
        else throw error;
      ...
      async.series([
        ...
        function spider(spiderDone) {
          zaproxy.spider.scan(zapTargetApp, maxChildren, zapApiKey,
            function(err, resp) {
        ...
        function includeInZapContext(includeInZapContextDone) {
          ...
          zaproxy.context.includeInContext("Default Context",
            "\\Q" + zapTargetApp + "\E.*", zapApiKey,
            function(err, resp) {
        ...
        function setForcedUserModeEnabled(
          setForcedUserModeEnabledDone) {
          var enabled = true;
          zaproxy.forcedUser.setForcedUserModeEnabled(enabled,
            zapApiKey,
            function(err, resp) {
        ...
        function newUser(newUserDone) {
          zaproxy.users.newUser(contextId, sutUserName, zapApiKey,
            function(err, resp) {
              userId = resp.userId;
        ...
        function setForcedUser(setForcedUserDone) {
          zaproxy.forcedUser.setForcedUser(contextId, userId,
            zapApiKey,
            function(err, resp) {
              setForcedUserDone(state.error);
            });
        },
        function setAuthenticationCredentials(
          setAuthenticationCredentialsDone) {
          zaproxy.users.setAuthenticationCredentials(
            contextId,
            userId,
            "username=" + sutUserName + "&" + "password=" +
            sutUserPassword,
            zapApiKey,
            function(err, resp) {
        ...
        function setUserEnabled(setUserEnabledDone) { // User should already be enabled?
          var enabled = true;
          zaproxy.users.setUserEnabled(contextId, userId, enabled,
            zapApiKey,
            function(err, resp) {
        ...
        function spiderAsUserForRoot(spiderAsUserForDone) {
          zaproxy.spider.scanAsUser(zapTargetApp, contextId, userId,
            maxChildren, zapApiKey,
            function(err, resp) {
            ...
            function(err, resp) {
              ...
              function status() {
                zaproxy.ascan.status(scanId, function(err, resp) {
                  ...
                  if (err) zapError = (err.code ===
                    "ECONNREFUSED") ? err : "";
                  zaproxy.core.numberOfAlerts(
                    zapTargetAppAndRoute,
                    function(err, resp) {
              ...
              zapInProgressIntervalId = setInterval(function() {
                ...
                if (zapError && statusValue !== String(100)) {
                  console.log(
                    "Canceling test. Zap API is unreachible."
                  );
                  ...
                  zaproxy.core.htmlreport(zapApiKey, function(
                    err, resp) {
                    var date = new Date();
                    var reportPath = __dirname +
                      ...
                      date.getMinutes() +
                      ".html";
                    console.log("Writing report to " +
                      reportPath);
                    fs.writeFile(reportPath, resp, function(
                      err) {
